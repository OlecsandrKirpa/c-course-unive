...
void conditionalOperator(int a) {
  printf("conditionalOperator with a = %d | a == 1 ? \"a is true\" : \"a is "
         "false\": %s\n\n",
...
void variablePointer() {
  ...
  char string = 'c';
  // Note that when you need to store the reference, you use the variable type
  // and add an asterisk at the end. If my variable is char, pointer will be
  // char*
...
int sum_array(int *arr, int arr_size) {
  ...
  return sum_array(arr, arr_size - 1) + arr[arr_size];
}

void test_sum_array(void) {
  printf("sum({1, 2, 3}: %d\n", sum_array((int[]){1, 2, 3}, 3));
  printf("sum({3, 3, 3}: %d\n", sum_array((int[]){3, 3, 3}, 3));
  printf("sum({3}: %d\n", sum_array((int[]){3}, 1));
}
